...
def runSimulator(data_length=1000, use_speed=True):
    ...
    if use_speed:
        recorded_data = np.vstack((data[3], data[4])) # [angle, signal1]
    else:
...
def recordRotations(rotations=1, data_length=1000, use_speed=True):
